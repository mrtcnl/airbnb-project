...
let resDiv = document.getElementById('reservation-window-container')
let scrollDiv = document.querySelector(".header-scroll-container")


window.onscroll = function() {resDivFunction()};


function resDivFunction() {
    if (window.scrollY >= 555) {
      resDiv.classList.add("sticky2")
      scrollDiv.style.display = "flex"
      scrollDiv.classList.add("sticky")
    } else {
      resDiv.classList.remove("sticky2")
...
function translateText() {
  let textElement = document.getElementById('translate-text')
  let buttonElement = document.getElementById('translate-button')
  ...
  if (isTranslated) {
    ...
    originalButton = buttonElement.innerHTML
    textElement.innerHTML = "Konforlu döşenmiş bir bölgesel  kulübede benzersiz bir tatilin tadını çıkarın. Tatra Dağları ve Tatra Milli Parkı'nın yakın çevresinde ağaçlarla çevrili, doğaya yakın, şehirden dinlenmek isteyenler için ideal bir yerdir. Çocuklu aileler, bir arkadaş grubu veya sadece bir çift için uygundur. Ayrıca köpeklerinizi, kedilerinizi, papağanlarınızı, kanaryalarınızı ve tatile götürmek istediğiniz diğer evcil hayvanlarınızı da ağırlıyoruz.";
    buttonElement.innerHTML = "Orijinal dilde göster"
    isTranslated = true;
  }
}
